...
const helper = {
  ...
  drawMembers: function ({
    ...
    // note - prepareToRender should be called before updating
    // the members state - this code assumes everything has
    ...
    if (showNetwork) {
      memberGroup.each(function (member) {
        ...
        // select just the planet to get the coordinates, since any lines added
        // to this group will expand the bounding box; this also ensures the line
        // will go directly to the center of the planet
  ...
  getLevelAtOffset: function ({ selection, levels, offset }) {
    var levelNow = this.selectedLevel({ selection });
    var nextLevelNumber = levelNow?.number + offset;
