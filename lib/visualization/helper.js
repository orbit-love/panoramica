...
const helper = {
  ...
  drawMembers: function ({
    ...
    members.assignVisualProperties();
    ...
    memberGroup.each(function (member) {
      ...
      // if the member has changed, we will stop the animation, update properties
      ...
      if (d3Selection.attr("transform") === null || member.reset) {
        d3Selection
          .attr("lastTime", member.position)
          .attr("currentTime", 0)
          .attr(
            "transform",
            helper.translate(member.position, pathNode, pathLength)
          );
        // remove the reset
        member.reset = false;
      }
      ...
      if (reset && animate) {
        helper.transition({ self, member, revolution });
      }
      ...
      if (selection && member.name === selection.name) {
        d3Selection.raise();
      }
    });
  },
  ...
  startAnimation: function ({ svgRef, revolution }) {
    ...
    memberGroup.each(function (member) {
      var self = this;
      if (!d3.active(self)) {
        helper.transition({ selection: self, member, revolution });
      }
    });
  },
  ...
  clearTransitions: function ({ svgRef }) {
