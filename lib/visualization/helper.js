...
const helper = {
  ...
  drawSun: function ({
    ...
    const sunRadius = Math.min(Math.max(ry(o1) - 30, 50), 80);
  ...
  generateMembers: function ({ levels, advocateCount }) {
    ...
    const connectionDensityFactor = 18;
    const connectionNumber = advocateCount * connectionDensityFactor;
    members.generateConnections({ number: connectionNumber });

    // TODO here - now that the connections are known, go set
    // the reach for the member, so that is always derived correctly
    // take the number of connections and translate to 1/2/3
    // will need to do by orbit level
    // const reachScale = d3.scaleLinear().range([1, 5]).domain([1, 2, 3]);
    members.list.forEach((member) => {
      const connections = members.getConnections({ member });
      member.reach = connections.length;
    });
