...
const helper = {
  ...
  drawSun: function ({
    ...
    const { offset, color, fontWeight, fontSize, label, step } =
      c.visualization.sun.text;
    const text = svg
    ...
    // use a tspan for multi-line text
    let parts = (simulation?.name || label).split(/\s/);
    text
      .selectAll("tspan")
      .data(parts)
      .join("tspan")
      .attr("dx", (_, i) => i * step)
      .attr("dy", (_, i) => i * fontSize * 1.25)
      .text((d) => d);
  },
  ...
  drawMembers: function ({
    ...
    var members = {
      list: memberHelper.prepareToRender({ community, levels, sort }),
    };
    ...
    memberGroup
      ...
      .attr("font-size", fontSize)
      .attr("font-weight", fontWeight)
      .attr("dx", (d) => d.planetSize + 10)
      .attr("dy", 5)
      .text((d) => d.actorName.split(" ")[0])
      .on("click", onClick);
    ...
    // just show it for now and use network toggle for big graph view
    if (true || showNetwork) {
      memberGroup.each(function (member) {
        ...
        if (member.connectionCount > 0 && selection?.id === member.id) {
          d3.select(self)
            .selectAll("line")
            .data(member.connectedMembers, (d) => d.id)
            .join("line")
            .attr("x1", 0)
            .attr("y1", 0)
            .attr("x2", (d) => {
          ...
          member.connectedMembers.forEach((connection) => {
  ...
  getNextMember: function ({ selection, level, community, toIndex }) {
  ...
  getPreviousMember: function ({ selection, level, community, toIndex }) {
    return this.getMemberAtOffset({
      selection,
      level,
      community,
  ...
  getMemberAtOffset: function ({
    ...
    var items = community.members;
    if (level) {
      items = community.members.filter(
        (member) => member.level === level.number
      );
    }
    var member;
  ...
  changeTransitionSpeed: function ({ community, levels, revolution }) {
    var helper = this;
