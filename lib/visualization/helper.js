  ...
  drawMembers: function ({
    ...
    if (false) {
      memberGroup.each(function (member) {
        ...
        if (selection?.id === member.id) {
          ...
          member.connections.forEach((connection) => {
            ...
            const line = d3Selection
              ...
              .attr("x1", 0)
              .attr("y1", 0)
              .attr("x2", -(sourceRect.x - targetRect.x))
              .attr("y2", -(sourceRect.y - targetRect.y))
        ...
        // console.log(member.id, bbox1, connection.id, bbox2);
      });
    }
  },
