...
const helper = {
  ...
  drawLevels: function ({
    ...
    svg
      ...
      .attr("opacity", (d) =>
        selection?.number === d.number || selection?.level?.number === d.number
          ? ringOpacityMaximum
          : d.ringOpacity * ringLabelOpacityMultiplier
      )
      .text((d) => d.name)
      .on("click", onClick);
    ...
    svg
      ...
      .attr("stroke-opacity", (d) =>
        selection?.number === d.number || selection?.level?.number === d.number
          ? ringOpacityMaximum
          : d.ringOpacity * ringOpacityMultiplier
      )
  ...
  drawSun: function ({
    ...
    svg
      ...
      .attr("stroke", stroke)
      .attr("fill", (_) =>
        selection && selection.name === "Mission" ? c.selectedColor : fill
      )
    ...
    const { offset, color, fontWeight } = c.visualization.sun.text;
    const text = svg
  ...
  drawMembers: function ({
    ...
    const { fontSize, fontWeight } = c.visualization.members;
    ...
    memberGroup.each(function (d) {
      ...
      if (scaledReach > 1) {
        d3Selection
          .append("circle")
          .attr("class", "planet clickable")
          .attr("r", d.reach === 2 ? d.planetSize + 3 : d.planetSize + 5)
          .attr("fill", d.planetColor)
          .attr("stroke", c.backgroundColor)
          .attr("stroke-width", (d) => (d.reach === 2 ? 0 : 3))
          .on("click", onClick);
      }
    });
    ...
    memberGroup
      ...
      .attr("stroke", c.backgroundColor)
      .attr("stroke-width", (d) => (c.scale123(d.reach) === 1 ? 0 : 3))
      .on("click", onClick);
