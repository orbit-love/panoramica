...
import MemberCollection from "data/member_collection";
...
const helper = {
  ...
  drawLevels: function ({ svgRef, selection, setSelection, setStep, levels }) {
    const svg = d3.select(svgRef.current);
  ...
  drawMembers: function ({ svgRef, members, selection, setSelection }) {
    var helper = this;
    const svg = d3.select(svgRef.current);
