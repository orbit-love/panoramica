...
export default class MemberReducer {
  ...
  reduce(activity) {
    var { actor, actorName, mentions, _ } = activity;
  ...
  finalize() {
    ...
    members.list = Object.values(this.memo);

    // first, set the OLs
    const activityCounts = members.list
    ...
    members.list.forEach((member) => {
      var expandedConnections = member.connections
        .map((connection) =>
          members.list.find((member) => member.actor === connection)
        )
        .filter((e) => e);
